...
export default function Navbar() {
  ...
  return (
    <nav className="bg-navy border-b border-white/10 sticky top-0 z-50">
      <div className="px-5 py-3.5 flex items-center justify-between max-w-3xl mx-auto">
        ...
        {user ? (
            ...
            <div className="flex items-center gap-2.5 pl-4 border-l border-white/10">
              {user.avatarUrl && (
                <img
                  src={user.avatarUrl}
                  alt={user.name}
                  className="w-7 h-7 rounded-full ring-2 ring-brand/40"
                />
              )}
              <button
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <Link
              to="/login"
              className="bg-brand text-white text-xs font-semibold px-4 py-1.5 rounded-lg"
            >
              Sign in
            </Link>
          </div>
        ...
        >
          <div className="space-y-1.5 w-5">
            <span
              className={`block h-0.5 bg-white rounded transition-all duration-200 ${open ? 'rotate-45 translate-y-2' : ''}`}
            />
            <span
              className={`block h-0.5 bg-white rounded transition-all duration-200 ${open ? 'opacity-0 scale-x-0' : ''}`}
            />
            <span
              className={`block h-0.5 bg-white rounded transition-all duration-200 ${open ? '-rotate-45 -translate-y-2' : ''}`}
            />
      ...
      {/* Mobile menu */}
      {open && (
        <div className="sm:hidden border-t border-white/10 bg-navy px-5 py-4">
          {user ? (
            <>
              ...
              <button
                onClick={handleLogout}
                className="mt-4 w-full py-2.5 rounded-xl text-sm font-medium text-red-400 border border-red-400/20 hover:bg-red-400/5 transition-colors"
              >
                Sign out
              </button>
            </>
            ...
            <div className="space-y-2">
              <Link
                to="/pricing"
                onClick={close}
                className="block text-center py-2.5 rounded-xl text-sm text-slate-300 border border-white/10 hover:bg-white/5 transition-colors"
              >
                Pricing
              </Link>
              <Link
                to="/login"
                onClick={close}
                className="block text-center bg-brand text-white text-sm font-semibold py-2.5 rounded-xl"
              >
                Sign in
              </Link>
            </div>
